...
export class EnvironmentManager {
  ...
  private loadConfig(): EnvironmentConfig {
    ...
    return {
      ...
      STRIPE_SECRET_KEY: this.getRequiredSecret('STRIPE_SECRET_KEY'),
      STRIPE_PUBLISHABLE_KEY: this.getRequiredSecret('STRIPE_PUBLISHABLE_KEY'),
      STRIPE_WEBHOOK_SECRET: this.getRequiredSecret('STRIPE_WEBHOOK_SECRET'),
  ...
  getSessionConfig() {
    return {
      ...
      maxAge: this.isDevelopment() ? 1000 * 60 * 60 * 24 * 7 : 1000 * 60 * 60 * 24, // 7 days dev, 1 day prod
    };
  }
}
